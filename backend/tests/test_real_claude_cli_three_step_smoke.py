...
TASK_ID = "smoke_real_cli_three_step_task"
...
def _workspace(tmp_path: Path) -> Path:
    ...
    store.create_workspace(
        ...
        task_markdown=(
            ...
            "Retrieval budget: 5\n\n"
            "This is a three-step real Claude CLI smoke test. The only permitted platform actions are:\n"
            "1. retrieve_sources\n"
            "2. create_evidence_seeds\n"
            "3. extract_evidence_cards\n\n"
            "Do not request enrichment, ranking, reports, landscape, gaps, ideas, experiments, claim ledger, "
        ...
        plan_markdown=(
            ...
            "The smoke test will call run_once exactly three times. It must not call run_until_stop.\n"
        ),
    )
...
def _three_step_prompt(snapshot: dict, registry: SkillRegistry | None = None, contract: dict | None = None) -> str:
    artifact_paths = {
        artifact.get("path")
    ...
    if "evidence/evidence_card_seeds.json" in artifact_paths:
        expected = _extract_cards_envelope()
        instruction = (
            "EvidenceCardSeed artifacts already exist. The only valid next action in this three-step smoke "
            "test is CALL_TOOL extract_evidence_cards. The input_artifacts array must include "
            "evidence/evidence_card_seeds.json. The output_artifacts array must include "
        ...
        instruction = (
            "Retrieval artifacts already exist. The only valid next action in this three-step smoke test is "
            "CALL_TOOL create_evidence_seeds."
        )
    else:
        expected = _retrieve_sources_envelope()
        instruction = (
            "No retrieval artifacts exist yet. The only valid next action in this three-step smoke test is "
            "CALL_TOOL retrieve_sources."
        )

    return (
        "You are inside a three-step research controller run_once smoke test.\n"
        ...
        "Do not request enrich_evidence_cards, rank_evidence, reports, landscape, gaps, ideas, experiments, "
        "claim ledger, manuscript, validation-only, or any future skill.\n"
...
def _retrieve_sources_envelope() -> dict:
    return {
        ...
        "decision": {
            ...
            "state_update_intent": {},
            "plan_update_intent": {},
            "reason": "The first step in the three-step smoke test is retrieving source candidates.",
            "warnings": [],
        },
        "notes": [],
        "warnings": [],
...
def _create_seeds_envelope() -> dict:
    return {
        ...
        "decision": {
            ...
            "state_update_intent": {},
            "plan_update_intent": {},
            "reason": "The second step in the three-step smoke test is creating EvidenceCardSeed artifacts.",
            "warnings": [],
        },
        "notes": [],
        "warnings": [],
...
def _extract_cards_envelope() -> dict:
    return {
        ...
        "decision": {
            ...
            "state_update_intent": {},
            "plan_update_intent": {},
            "reason": "The third step in the three-step smoke test is creating initial Evidence Cards.",
            "warnings": [],
        },
        "notes": [],
        "warnings": [],
...
@pytest.mark.skipif(
    ...
    reason="Real Claude Code CLI three-step smoke test is opt-in only",
)
def test_real_claude_cli_three_step_retrieve_seed_extract_smoke(monkeypatch, tmp_path: Path) -> None:
    ...
    warnings = list(first.warnings) + list(second.warnings) + list(third.warnings)
    for result in (first, second, third):
        if result.skill_result:
    ...
    summary = {
        ...
        "violations": _violations(first, second, third),
        "warnings": list(dict.fromkeys(warnings)),
    }
    print("REAL_CLAUDE_CLI_THREE_STEP_SMOKE " + json.dumps(summary, ensure_ascii=False, sort_keys=True))
    ...
    assert executed_skills == ["retrieve_sources", "create_evidence_seeds", "extract_evidence_cards"]
