...
TASK_ID = "smoke_real_cli_screening_decision_task"
...
def _workspace(tmp_path: Path) -> Path:
    ...
    store.create_workspace(
        ...
        task_markdown=(
            "# Task\n\n"
            f"Topic: {TOPIC}\n\n"
            "This is a bounded screening decision smoke test. The workspace already has candidate ideas, "
            "gap map, landscape, enriched Evidence Cards, ranked evidence, diagnostics, and a minimal report. "
        ...
        plan_markdown=(
            ...
            "- screen_novelty_feasibility_risk: pending\n\n"
            "The next valid explicit step is screen_novelty_feasibility_risk.\n"
        ),
    )
    ...
    _write_json(
        ...
        "ideas/candidate_ideas.json",
        {
            ...
            "task_id": TASK_ID,
            "idea_set_id": "candidate_ideas_smoke_screening",
            "gap_map_id": "gap_map_smoke_screening",
            "landscape_id": "landscape_smoke_screening",
            "ideas": [
                {
                    "idea_id": "idea_001",
                    "title": "Vacancy-controlled alkaline HER comparison",
                    "summary": "A candidate direction grounded in a comparison gap and selected evidence.",
                    "idea_type": "comparative_study",
                    "gap_basis": {"gap_ids": ["gap_001"], "evidence_ids": ["ecard_001"]},
                    "evidence_basis": {
    ...
    _write_json(
        ...
        "ideas/idea_generation_diagnostics.json",
        {
            ...
            "warnings": ["screening_smoke_fixture"],
        },
    )
    _write_text(
        workspace_root,
...
def _screening_decision_prompt(
    snapshot: dict,
    ...
    expected = {
        ...
        "decision": {
            ...
            "skill_request": {
                ...
                "reason": (
                    "The explicit screening plan has the required candidate ideas, gap map, landscape, "
                    "enriched evidence, ranked evidence, and diagnostics artifacts, so the next bounded "
                    "action is to run screening."
                ),
            },
            "artifact_refs": [
                "ideas/candidate_ideas.json",
            ...
            "state_update_intent": {},
            "plan_update_intent": {},
            "reason": "The explicit screening step should run after validated candidate idea artifacts exist.",
            "warnings": [],
        },
        "notes": [],
        "warnings": [],
    ...
    return (
        ...
        "- ranked_evidence/coverage_diagnostics.json\n"
        "- reports/minimal_topic_to_evidence_report.json\n"
        "The explicit screening plan requires the next skill: screen_novelty_feasibility_risk.\n"
        ...
        "Do not generate screening content.\n"
        ...
        "Do not claim risk is finalized.\n"
        "Do not modify files. Do not run shell commands. Do not access or mutate database.\n"
        "Do not use retrieval/... as an input artifact.\n"
        "Do not request create_experiment_matrix, build_claim_ledger, draft_manuscript_section, or any P2-M12+ skill.\n"
...
@pytest.mark.skipif(
    ...
    reason="Real Claude Code CLI screening decision smoke test is opt-in only",
)
def test_real_claude_cli_screening_decision_contract_smoke(monkeypatch, tmp_path: Path) -> None:
    ...
    candidate_ideas_json_present = "ideas/candidate_ideas.json" in input_artifacts
    screening_input_errors = validate_screening_input_artifacts(input_artifacts)
    p2_m12_plus_requested = skill_name in P2_M12_PLUS_SKILLS
    result = (
        "passed"
        if (
            format_issue == "none"
            ...
            and skill_name == "screen_novelty_feasibility_risk"
            and input_artifacts == INPUT_ARTIFACTS
            and output_artifacts == OUTPUT_ARTIFACTS
            and not retrieval_input_present
            and candidate_ideas_json_present
            and not screening_input_errors
            and not p2_m12_plus_requested
        )
    ...
    summary = {
        ...
        "candidate_ideas_json_present": candidate_ideas_json_present,
        "screening_input_errors": screening_input_errors,
        "p2_m12_plus_requested": p2_m12_plus_requested,
    ...
    print("REAL_CLAUDE_CLI_SCREENING_DECISION_SMOKE " + json.dumps(summary, ensure_ascii=False, sort_keys=True))
    ...
    assert candidate_ideas_json_present is True
    assert screening_input_errors == []
    assert p2_m12_plus_requested is False
    ...
    assert not (workspace_root / "screening/idea_screening_results.md").exists()
    assert not (workspace_root / "screening/screening_diagnostics.json").exists()
