...
TASK_ID = "smoke_real_cli_five_step_task"
...
def _workspace(tmp_path: Path) -> Path:
    ...
    store.create_workspace(
        ...
        task_markdown=(
            ...
            "Retrieval budget: 5\n\n"
            "This is a five-step real Claude CLI smoke test. The only permitted platform actions are:\n"
            "1. retrieve_sources\n"
            "2. create_evidence_seeds\n"
            "3. extract_evidence_cards\n"
            "4. enrich_evidence_cards\n"
            "5. rank_evidence\n\n"
            "Do not request reports, landscape, gaps, ideas, experiments, claim ledger, or manuscript skills.\n"
        ),
        plan_markdown=(
            "# Plan\n\n"
            ...
            "The smoke test will call run_once exactly five times. It must not call run_until_stop.\n"
        ),
    )
...
def _five_step_prompt(snapshot: dict, registry: SkillRegistry | None = None, contract: dict | None = None) -> str:
    artifact_paths = {
        artifact.get("path")
    ...
    if "evidence/evidence_cards.enriched.json" in artifact_paths:
        expected = _rank_evidence_envelope()
        instruction = (
            "Enriched Evidence Cards already exist. The only valid next action in this five-step smoke test is "
            "CALL_TOOL rank_evidence. The input_artifacts array must include evidence/evidence_cards.enriched.json. "
            "The output_artifacts array must include ranked_evidence/evidence_selection.json and "
        ...
        instruction = (
            "Initial Evidence Cards already exist. The only valid next action in this five-step smoke test is "
            "CALL_TOOL enrich_evidence_cards."
        )
    elif "evidence/evidence_card_seeds.json" in artifact_paths:
        expected = _extract_cards_envelope()
        instruction = (
            "EvidenceCardSeed artifacts already exist. The only valid next action in this five-step smoke test is "
        ...
        instruction = (
            "Retrieval artifacts already exist. The only valid next action in this five-step smoke test is "
            "CALL_TOOL create_evidence_seeds."
        )
    else:
        expected = _retrieve_sources_envelope()
        instruction = (
            "No retrieval artifacts exist yet. The only valid next action in this five-step smoke test is "
            "CALL_TOOL retrieve_sources."
        )

    return (
        "You are inside a five-step research controller run_once smoke test.\n"
        ...
        "Do not request reports, landscape, gaps, ideas, experiments, claim ledger, manuscript, validation-only, "
        "or any future skill.\n"
...
def _retrieve_sources_envelope() -> dict:
    return {
        ...
        "decision": {
            ...
            "state_update_intent": {},
            "plan_update_intent": {},
            "reason": "The first step in the five-step smoke test is retrieving source candidates.",
            "warnings": [],
        },
        "notes": [],
        "warnings": [],
...
def _create_seeds_envelope() -> dict:
    return {
        ...
        "decision": {
            ...
            "state_update_intent": {},
            "plan_update_intent": {},
            "reason": "The second step in the five-step smoke test is creating EvidenceCardSeed artifacts.",
            "warnings": [],
        },
        "notes": [],
        "warnings": [],
...
def _extract_cards_envelope() -> dict:
    return {
        ...
        "decision": {
            ...
            "state_update_intent": {},
            "plan_update_intent": {},
            "reason": "The third step in the five-step smoke test is creating initial Evidence Cards.",
            "warnings": [],
        },
        "notes": [],
        "warnings": [],
...
def _enrich_cards_envelope() -> dict:
    return {
        ...
        "decision": {
            ...
            "state_update_intent": {},
            "plan_update_intent": {},
            "reason": "The fourth step in the five-step smoke test is enriching initial Evidence Cards.",
            "warnings": [],
        },
        "notes": [],
        "warnings": [],
...
def _rank_evidence_envelope() -> dict:
    return {
        ...
        "decision": {
            ...
            "state_update_intent": {},
            "plan_update_intent": {},
            "reason": "The fifth step in the five-step smoke test is ranking and selecting representative evidence.",
            "warnings": [],
        },
        "notes": [],
        "warnings": [],
...
def _ranking_validation_status(result) -> str | None:
    for validation in result.validation_results:
        if validation.artifact_type == "evidence_selection":
            return validation.validation_status.value
    return None


@pytest.mark.skipif(
    os.getenv("RUN_REAL_CLAUDE_CLI_TEST") != "1",
    reason="Real Claude Code CLI five-step smoke test is opt-in only",
)
def test_real_claude_cli_five_step_retrieve_seed_extract_enrich_rank_smoke(monkeypatch, tmp_path: Path) -> None:
    ...
    warnings = list(first.warnings) + list(second.warnings) + list(third.warnings) + list(fourth.warnings) + list(fifth.warnings)
    for result in (first, second, third, fourth, fifth):
        if result.skill_result:
    ...
    summary = {
        "real_cli_invoked": all(
            ...
            for response in (first_response, second_response, third_response, fourth_response, fifth_response)
        ),
        ...
        "fifth_exit_code": fifth_response.exit_code,
        ...
        "fifth_timed_out": fifth_response.timed_out,
        ...
        "rank_result_status": fifth.skill_result.status.value if fifth.skill_result else None,
        "ranking_validation_status": _ranking_validation_status(fifth),
        "violations": _violations(first, second, third, fourth, fifth),
        "warnings": list(dict.fromkeys(warnings)),
    }
    print("REAL_CLAUDE_CLI_FIVE_STEP_SMOKE " + json.dumps(summary, ensure_ascii=False, sort_keys=True))

    assert fifth.status == ControllerStepStatus.SUCCESS
    assert _parse_success(fifth) is True
    assert _validation_success(fifth) is True
    assert _decision_type(fifth) == "CALL_TOOL"
    assert _skill_name(fifth) == "rank_evidence"
    assert fifth.skill_result is not None
    assert fifth.skill_result.status == SkillExecutionStatus.SUCCESS
    assert fifth.skill_result.input_artifacts == ["evidence/evidence_cards.enriched.json"]
    assert fifth.skill_result.output_artifacts == [
        "ranked_evidence/evidence_selection.json",
        "ranked_evidence/coverage_diagnostics.json",
    ]
    assert _ranking_validation_status(fifth) in {
        ValidationStatus.PASSED.value,
        ValidationStatus.DEGRADED_WITH_WARNING.value,
    }
    ...
    assert (workspace_root / "ranked_evidence/evidence_selection.json").exists()
    assert (workspace_root / "ranked_evidence/coverage_diagnostics.json").exists()
    ...
    assert "ranked_evidence/evidence_selection.json" in manifest_artifacts
    assert "ranked_evidence/coverage_diagnostics.json" in manifest_artifacts
    assert len(controller_events) >= 5
    ...
    assert "rank_evidence" in state["completed_steps"]
    ...
    assert "rank_evidence" in plan_md
